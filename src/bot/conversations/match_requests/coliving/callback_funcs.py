...
import conversations.match_requests.coliving.states as states
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    Проверяем, одобрен ли переход к профилю (sender)
    Вычисляем пользователя, полившего лайк (receiver)
...
async def link_sender_to_receiver(
    ...
    await api_service.update_status_coliving_like(
        pk=like_id,
...
async def _get_like_id_and_sender_id(
    ...
    """Возвращает id ColivingLike для дальнейшего формирования запроса к API."""
